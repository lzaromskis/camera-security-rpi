...
class OpenCVImageDrawer(IImageDrawer):
    def DrawRectangle(self, frame: Frame, bounds: BoundingBox, colour: Tuple[int, int, int], thickness: int):
        data = frame.GetData()
        height, width = OpenCVImageDrawer.__GetDimensions(data)
    ...
    def DrawText(self, frame: Frame, text: str, pos: Tuple[float, float], font: int, font_scale: int, font_thickness: int,
        ...
        if background_colour is not None:
            start_pos = (width_pixels, height_pixels - text_h)
            end_pos = (width_pixels + text_w, height_pixels)
            cv2.rectangle(data, start_pos, end_pos, background_colour, -1)
        cv2.putText(data, text, (width_pixels, height_pixels), font, font_scale, text_colour, font_thickness)
